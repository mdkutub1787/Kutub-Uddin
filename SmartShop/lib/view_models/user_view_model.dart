...
class UserViewModel extends ChangeNotifier {
  ...
  void fetchUsers({String? shopId, bool isSuperAdmin = false}) {
    _isLoading = true;
    _dbRef.onValue.listen((event) {
      final Map<dynamic, dynamic>? data = event.snapshot.value as Map<dynamic, dynamic>?;
      if (data != null) {
        List<UserModel> allUsers = data.entries.map((e) {
        ...
        if (isSuperAdmin) {
          ...
          // Admin sees users who belong to their shop
          _users = allUsers.where((u) => u.shopId == shopId).toList();
        } else {
          _users = [];
        }
      } else {
