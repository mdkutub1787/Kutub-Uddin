package com.logicsoftbd.lsl.ui.v_1_ui.finish_fabric.finish_fabric_receive_old;
...
public class V1_FinishFabricReceiveActivity extends AppCompatActivity implements View.OnClickListener, V1_FinishReceiveRecyclerAdapter.OnDeleteSelectListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v1_finish_fabric_receive);
...
//            String[] roomRackArray = finish_receive_room_rack_scan.split("-", 2); // Limit to 2 parts
    ...
    private void requestForBarcode(String barcodeScan) {
        _pdialog.show();
        apiInterface.getGreyRollReceiveByBarcodeModelClassCall(barcodeScan, room_rack).enqueue(new Callback<V1_GreyRollReceiveRequest>() {
            @Override
            public void onResponse(@NonNull Call<V1_GreyRollReceiveRequest> call, @NonNull Response<V1_GreyRollReceiveRequest> response) {
                _pdialog.dismiss();
                Log.d(TAG, "onResponse: "+response.toString());
                if(response.isSuccessful()){
                    if(!response.body().getResultset().getMasterPart().getStatus().equals("Failed")){
                        if (response.body() != null && response.body().getResultset().getDtlsPart().size() > 0) {
                            company_id = Integer.parseInt(response.body().getResultset().getMasterPart().getCompanyId());
                            delivery_number = response.body().getResultset().getMasterPart().getSysNumber();
                            for (int i = 0; i < response.body().getResultset().getDtlsPart().size(); i++) {
                                if(dataList.size() > 0 && !finishFabricReceiveItemModels.get(i).getBATCH_ID().equals(response.body().getResultset().getDtlsPart().get(i).getSysNumber())){
                                    showAlertMessage("Multiple challan can't scan", 0, 0);
                                    break;
                                }
                                boolean s = false;
                                if(dataList.size() > 0){
                                    for(int j=0; j<finishFabricReceiveItemModels.size(); j++){
                                        if(finishFabricReceiveItemModels.get(j).getBARCODE_NO().equals(_barcodeScanTV.getText().toString())){
                                ...
                                if(s == true)
                                    break;
                                V1_GreyRollReceiveItemModel finish_receiveRollReceiveItemModel = new V1_GreyRollReceiveItemModel();
                                finish_receiveRollReceiveItemModel.setBarcodeNo(response.body().getResultset().getDtlsPart().get(i).getBarcodeNo());
                                finish_receiveRollReceiveItemModel.setRollNo(response.body().getResultset().getDtlsPart().get(i).getRollNo());
                                finish_receiveRollReceiveItemModel.setQnty(response.body().getResultset().getDtlsPart().get(i).getQnty().trim());
                                finish_receiveRollReceiveItemModel.setJobNo(response.body().getResultset().getDtlsPart().get(i).getJobNo());
                                finish_receiveRollReceiveItemModel.setBookingNo(response.body().getResultset().getDtlsPart().get(i).getBookingNo());
                                finish_receiveRollReceiveItemModel.setProgramNo(response.body().getResultset().getDtlsPart().get(i).getProgramNo());
                                finish_receiveRollReceiveItemModel.setConstruction(response.body().getResultset().getDtlsPart().get(i).getConstruction());
                                finish_receiveRollReceiveItemModel.setComposition(response.body().getResultset().getDtlsPart().get(i).getComposition());
                                finish_receiveRollReceiveItemModel.setGsm(response.body().getResultset().getDtlsPart().get(i).getGsm());
                                finish_receiveRollReceiveItemModel.setWidth(response.body().getResultset().getDtlsPart().get(i).getWidth());
                                finish_receiveRollReceiveItemModel.setColorName(response.body().getResultset().getDtlsPart().get(i).getColorName());
                                finish_receiveRollReceiveItemModel.setYarnLot(response.body().getResultset().getDtlsPart().get(i).getYarnLot());
                                finish_receiveRollReceiveItemModel.setStitchLength(response.body().getResultset().getDtlsPart().get(i).getStitchLength());
                                finish_receiveRollReceiveItemModel.setBrandId(response.body().getResultset().getDtlsPart().get(i).getBrandId());
                                finish_receiveRollReceiveItemModel.setSysNumber(response.body().getResultset().getDtlsPart().get(i).getSysNumber());
                                finish_receiveRollReceiveItemModel.setMachineNo(response.body().getResultset().getDtlsPart().get(i).getMachineName());
                                finish_receiveRollReceiveItemModel.setStatus(false);
//                                finishFabricReceiveItemModels.add(finish_receiveRollReceiveItemModel);
                            }
                            finishReceiveRecyclerAdapter.notifyDataSetChanged();
                        }
                    }else{
                        _barcodeScanTV.setText("");
                        showAlertMessage(response.body().getResultset().getMasterPart().getMsg(), 0, 0);
                    }
                }
            }
    ...
    private void showAlertMessage(String msg, int i, int position) {
        AlertDialog.Builder builder = new AlertDialog.Builder(V1_FinishFabricReceiveActivity.this);
