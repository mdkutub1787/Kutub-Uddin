...
import com.logicsoftbd.lsl.data.network.v1_model.V1_SewingDefectModel;
...
import com.logicsoftbd.lsl.ui.v_1_ui.style_wise_sewing.V1_SewingOutputDefectRecyclerAdapter;
...
public class V1_SewingOutputActivity_v1 extends AppCompatActivity implements View.OnClickListener,
        V1_SewingOutputRecyclerViewAdapter_v1.OnRejectHeadListener,
        V1_SewingOutputRecyclerViewAdapter_v1.OnDefectHeadListener,
        V1_BundleWiseSewingOperationRecyclerAdapter.OnOperationSelectListener,
        V1_SewingOutputDefectRecyclerAdapter.OnDefectSelectListener,
    ...
    private Spinner sCompany, sSource, sSewingCompany, sLocation, sFloor, sLineNo, sSfhift;
    ...
    public int isOrganic = 0, updatedID = 0, rescan = 0, color_type_id = 0, sewing_input_line = 0, companyId = 0, shiftId = 0, sewingcompanyId = 0,
    ...
    private V1_SewingOutputRecyclerViewAdapter_v1 sewingOutputRecyclerViewAdapter;
    private V1_BundleWiseSewingOperationRecyclerAdapter bundleWiseSewingOperationRecyclerAdapter;
    private V1_SewingOutputDefectRecyclerAdapter sewingOutputDefectRecyclerAdapter;
    ...
    private List<V1_AlterSewingDefectResponse.DefectType> defectPopUPModelClasses;

    public static ArrayList<V1_SewingDefectModel> sewingDefectModels = new ArrayList<>();
    public static ArrayList<V1_SewingDefectModel> sewingAlterModels = new ArrayList<>();
    public static ArrayList<V1_SewingDefectModel> sewingSpotModels = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v1_sewing_output_v1);
...
//        updatemenu = intent.getIntExtra("u", 0);
        ...
        if (barcodeNumber != null && type_entry.equals("bundle_wise_sewing_output_v3"))
        {
            ...
            {
                {
                    ...
                    if(new V1_SewingOutputDBAdapter(V1_SewingOutputActivity_v1.this).saveLoginData(l))
                    {
...
//            requestForSewingOperation();
    ...
    private void initRecyclerView() {
        ...
        sewingOutputRecyclerViewAdapter = new V1_SewingOutputRecyclerViewAdapter_v1(modelArrayList, this, this, getApplicationContext());
        ...
        apiInterface.getBundleWiseCompanyToShiftClassCall().enqueue(new Callback<V1_BundleWiseCompanyToShiftClass>() {
            ...
            public void onResponse(Call<V1_BundleWiseCompanyToShiftClass> call, Response<V1_BundleWiseCompanyToShiftClass> response) {
                ...
                if(response.isSuccessful())
                {
                    try {
                        shiftNameList.clear();
                        loadDataShift(response);
                    }catch (Exception e){
                        Log.d(TAG, "onResponse: "+e.getMessage()+e);
                        Toast.makeText(V1_SewingOutputActivity_v1.this, "Shift data not found.", Toast.LENGTH_SHORT).show();
                    }
                    
                }
            }

            private void loadDataShift(Response<V1_BundleWiseCompanyToShiftClass> response) {
                V1_BundleWiseCompanyToShiftClass.Datum shiftName;
                List<V1_BundleWiseCompanyToShiftClass.Datum> locationses = response.body().getData();
                shiftNameList.add(0, "--Select Shift--");
                for(V1_BundleWiseCompanyToShiftClass.Datum d : locationses)
                {
                    shiftName = d;
                    final V1_BundleWiseCompanyToShiftClass.Datum ShiftName = shiftName;
                    shiftNameList.add(String.valueOf(ShiftName.getShift()));
                }
                shiftNameArray = new String[shiftNameList.size()];
                for(int i = 0; i < shiftNameList.size(); i++)
                {
                    shiftNameArray[i] = shiftNameList.get(i);
                }

                V1_BundleWiseCompanyToShiftClass.Datum shiftId;
                List<V1_BundleWiseCompanyToShiftClass.Datum> shiftID = response.body().getData();
                shiftNameId.add(0, 0);
                for(V1_BundleWiseCompanyToShiftClass.Datum d : shiftID)
                {
                    shiftId = d;
                    final V1_BundleWiseCompanyToShiftClass.Datum ShiftId = shiftId;
                    shiftNameId.add(ShiftId.getId());
                }
            ...
            public void onFailure(Call<V1_BundleWiseCompanyToShiftClass> call, Throwable t) {
                if (t instanceof IOException) {
                    Toast.makeText(V1_SewingOutputActivity_v1.this, "No network available, please check your WiFi or Data connection", Toast.LENGTH_SHORT).show();
                    hideDialog();
                }
                else {
    ...
    private void setAdapterData_Shift() {
        ArrayAdapter<String> adapterLocation = new ArrayAdapter<String>(V1_SewingOutputActivity_v1.this, R.layout.sewing_spinner_layout, shiftNameArray);
        sSfhift.setAdapter(adapterLocation);
...
//                   int SHIFT_ID = shiftId;
            ...
            case R.id.saveBT:
                if(locationId != 0 && floorId != 0 && lineId != 0&& shiftId !=0)
                {
                    ...
                    if(modelArrayList.get(0).getAlter() <= 0){
                        for(int i = 0; i< sewingDefectModels.size(); i++){
                            sewingDefectModels.get(i).setDefectCount("0");
                        }
                        if(sewingOutputDefectRecyclerAdapter != null)
                            sewingOutputDefectRecyclerAdapter.notifyDataSetChanged();
                    }
                    ...
                    for(int i = 0; i < sewingDefectModels.size(); i++)
                    {
                        if(sewingDefectModels.get(i).getDefectType() == 3){
                            if(sewingDefectModels.get(i).getDefectCount() != null && !sewingDefectModels.get(i).getDefectCount().equals("") && !sewingDefectModels.get(i).getDefectCount().equals("0")){
                                alter_defect += sewingDefectModels.get(i).getId()+"*"+ sewingDefectModels.get(i).getDefectCount()+"__";
                            }
                        }
                        if(sewingDefectModels.get(i).getDefectType() == 4){
                            if(sewingDefectModels.get(i).getDefectCount() != null && !sewingDefectModels.get(i).getDefectCount().equals("") && !sewingDefectModels.get(i).getDefectCount().equals("0")){
                                spot_defect += sewingDefectModels.get(i).getId()+"*"+ sewingDefectModels.get(i).getDefectCount()+"__";
                            }
                        }
                    ...
                    }else if(modelArrayList.get(0).getAlter() > 0 && modelArrayList.get(0).getSpot() > 0 && (alter_defect.equals("") || spot_defect.equals(""))){
                        showAlertMessage("Defect not selected.", 0);
                    }else if(modelArrayList.get(0).getAlter() > 0 && !countTotalSelectedAlterDefect(sewingDefectModels)){
                        showAlertMessage( "Selected outside defect can't less from inside alter defect.", 0);
                    ...
                    ((TextView)sSfhift.getSelectedView()).setError("Please fill the shift name");
                }
                break;
            case R.id.refreshBT:
                initialization();
                ...
                break;
            case R.id.toolbar:
                Intent intent = V1_MenuActivity.getStartIntent(V1_SewingOutputActivity_v1.this);
    ...
    private boolean countTotalSelectedAlterDefect(ArrayList<V1_SewingDefectModel> sewingDefectModels) {
        int count = 0;
        for(V1_SewingDefectModel item: sewingDefectModels){
    ...
    private void barcodeScanMethod() {
        ...
        if(!barcode.isEmpty())
        {
            ...
            intent.putExtra("qc", "bundle_wise_sewing_output_v3");
            startActivity(intent);
            finish();
        }
    }
...
//                    }
            ...
            public void onFailure(Call<V1_DataSaveResponse> call, Throwable t) {
                ...
                Toast.makeText(V1_SewingOutputActivity_v1.this, "Failed", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//                        }
    ...
    @Override
    public void onDefectHeadClick(int position, View v) {
        if(modelArrayList.get(0).getAlter() > 0) {
//            Intent intent = new Intent(v.getContext(), V1_SewingOutputOperationActivity.class);
...
//            v.getContext().startActivity(intent);
            ...
            showAlertMessage("Please enter \"Defect\" quantity.", 0);
        }
    }
...
//                        }
                        ...
                        for(int i=0; i<response.body().getResultset().getDefectType().size(); i++){
                            V1_SewingDefectModel sewingSpotModel = new V1_SewingDefectModel();
                            String sentence = response.body().getResultset().getDefectType().get(i).getDefectName();
                            boolean isNumber = sentence.matches("\\d+");
                            if(isNumber){
                            ...
                            sewingSpotModel.setDefectType(Integer.valueOf(s));
//                            sewingSpotModels.add(sewingSpotModel);
                            sewingDefectModels.add(sewingSpotModel);
                        }
                    }
                }
            }
...
//                        }
                        ...
                        for(int i=0; i<response.body().getResultset().getDefectType().size(); i++){
                            V1_SewingDefectModel sewingAlterModel = new V1_SewingDefectModel();
                            String sentence = response.body().getResultset().getDefectType().get(i).getDefectName();
                            boolean isNumber = sentence.matches("\\d+");
                            if(isNumber){
                            ...
                            sewingAlterModel.setDefectType(Integer.valueOf(s));
//                            sewingAlterModels.add(sewingAlterModel);
                            sewingDefectModels.add(sewingAlterModel);
                        }
                    }
                }
            }
    ...
    private void setSewingDefectData() {
        if(defectType.equals("A")) {
            sewingOutputDefectRecyclerAdapter = new V1_SewingOutputDefectRecyclerAdapter(sewingDefectModels, this,  this);
            defectRecyclerView.setAdapter(sewingOutputDefectRecyclerAdapter);
...
//                        }
    ...
    private void  timePickerMethor() {
        ...
        mTimePicker = new TimePickerDialog(V1_SewingOutputActivity_v1.this, new TimePickerDialog.OnTimeSetListener() {
            @Override
...
//        finish();
    ...
    private void showAlertMessage(String msg, int i) {
        AlertDialog.Builder builder = new AlertDialog.Builder(V1_SewingOutputActivity_v1.this);
        builder.setTitle("Message")
                ...
                .setPositiveButton("Ok", (dialog, which) -> {
                    if(i == 1){
                        ...
                        alterSewingOutputOperationItemModels.clear();
                        spotSewingOutputOperationItemModels.clear();
                        Intent intent = new Intent(V1_SewingOutputActivity_v1.this, V1_QRBarcodeScannerActivity.class);
                        intent.putExtra("url", urladdress);
                        intent.putExtra("qc", "bundle_wise_sewing_output_v3");
                        startActivity(intent);
                        finish();
