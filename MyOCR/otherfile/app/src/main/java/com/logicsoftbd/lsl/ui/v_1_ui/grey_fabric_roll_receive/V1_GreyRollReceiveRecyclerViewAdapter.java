...
public class V1_GreyRollReceiveRecyclerViewAdapter extends RecyclerView.Adapter<V1_GreyRollReceiveRecyclerViewAdapter.ViewHolder> {
    ArrayList<V1_GreyRollReceiveItemModel> greyRollReceiveItemModels = new ArrayList<>();
    Context context;

    private OnMoreHeadListener mOnMoreHeadListener;
    private OnRemoveHeadListener mOnRemoveHeadListener;

    public V1_GreyRollReceiveRecyclerViewAdapter(ArrayList<V1_GreyRollReceiveItemModel> greyRollReceiveItemModels, OnMoreHeadListener mOnMoreHeadListener, OnRemoveHeadListener mOnRemoveHeadListener, Context context) {
    ...
    @NonNull
    @Override
    public V1_GreyRollReceiveRecyclerViewAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int i) {
...
//        viewHolder.fsoJobTV.setText(greyRollReceiveItemModels.get(position).getJobNo().substring(greyRollReceiveItemModels.get(position).getJobNo().length()-4));
        viewHolder.fabBookingTV.setText(greyRollReceiveItemModels.get(position).getBookingNo());
        viewHolder.programNoTV.setText(greyRollReceiveItemModels.get(position).getProgramNo());
        viewHolder.internalRefTV.setText(greyRollReceiveItemModels.get(position).getInternalRef());
        ...
        viewHolder.brandTV.setText(String.valueOf(greyRollReceiveItemModels.get(position).getBrandId()));
        viewHolder.machineNameTV.setText(String.valueOf(greyRollReceiveItemModels.get(position).getMachineNo()));

        if(greyRollReceiveItemModels.get(position).getStatus()){
            ...
            viewHolder.internalRefF.setVisibility(View.VISIBLE);
            ...
            viewHolder.internalRefF.setVisibility(View.GONE);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        private TextView slTV, barcodeTV, rollTV, rollWgtTV, fsoJobTV, fabBookingTV, programNoTV, internalRefTV, machineNameTV, compositionTV, constructionTV, gsmTV, diaTV, fabricColorTV, yarnLotTV, stitchLengthTV, brandTV,
                slF, barcodeF, rollF, rollWgtF, fsoJobF, fabBookingF, programNoF, internalRefF, machineNameF, compositionF, constructionF, gsmF, diaF, fabricColorF, yarnLotF, stitchLengthF, brandF, removeF, moreF;
        ...
        public ViewHolder(@NonNull View itemView, OnMoreHeadListener mOnMoreHeadListener, OnRemoveHeadListener mOnRemoveHeadListener) {
            ...
            internalRefTV = itemView.findViewById(R.id.internalRefTV);
            ...
            internalRefF = itemView.findViewById(R.id.internalRefF);
