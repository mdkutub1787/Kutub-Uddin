...
public class CompactingRollWiseActivity extends AppCompatActivity implements View.OnClickListener, FinishProductionCompactingRecyclerAdapter.OnSelectSelectListener {
    private static final String TAG = "CompactingRollWiseActiv";
    ...
    private EditText _batchNoScanET, _barcodeScanET, _overFeedET, _temperatureET, _lengthShrinkageET, _widthShrinkageET, _spiralityET, _speedET, _steamET, _productionTimeHour, _productionTimeMin, _processStartTimeHour, _processStartTimeMin, _processEndTimeHour, _processEndTimeMin, _reCompactingET;
    ...
    private FinishProductionCompactingRecyclerAdapter finishProductionCompactingRecyclerAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_compacting_roll_wise);
    ...
    private void fetchCompactingData() {
        progressBarState();
        finishProductionViewModel.getFinishProductionDefaultResponse("33").observe(this, apiResponse -> {
            if(apiResponse != null){
                try{
                ...
                DialogHelper.showErrorDialog(CompactingRollWiseActivity.this, "Message", "Something went wrong!");
            }
        });
    }
    ...
    private void fetchCompactingBatchScan(String scannedBarcode) {
        progressBarState();
        finishProductionViewModel.getFinishProductionCompactingBatchScanResponse(scannedBarcode, "33").observe(this, apiResponse -> {
            if(apiResponse!= null){
                if(apiResponse.getResultset() != null){
                    ...
                    compactBatchScanResponse = apiResponse;
    ...
    private void setDataToUI(CompactBatchScanResponse response) {
        ...
        if(response.getResultset().getInputAreaIndex().getReSlittingNo().equals("false")){
            _reCompactingET.setText("0");
        }else{
            _reCompactingET.setText(response.getResultset().getInputAreaIndex().getReSlittingNo());
        }


        Log.d(TAG, "setDataToUI: "+floorId +" "+machineId+" "+barcodeScan);
...
//        _sourceNameSpinner.setSelection(saved_source_position);
    ...
    private void setUpProcessSpinner() {
        ...
        _processNameSpinner.setSelection(processNameList.indexOf("Compacting"));
    ...
    private void setupCompactingProdScanRecyclerView() {
        ...
        finishProductionCompactingRecyclerAdapter = new FinishProductionCompactingRecyclerAdapter( dtlsIndexArrayList, this, this);
        _finishProductionReceiveRecyclerView.setAdapter(finishProductionCompactingRecyclerAdapter);
    }
    ...
    private void saveRequestCompactingObject() throws JSONException {
        ...
        index_obj.put("RE_SLITING_NO", compactBatchScanResponse.getResultset().getInputAreaIndex().getReSlittingNo());
...
//        index_obj.put("END_MINUTES", _processEndTime.getText().toString().split(":")[1]);
        ...
        index_obj.put("WIDTH_SHRINKAGE", _widthShrinkageET.getText().toString());
        index_obj.put("LENGTH_SHRINKAGE", _lengthShrinkageET.getText().toString());
        ...
        index_obj.put("TEMPARATURE", _temperatureET.getText().toString());
        ...
        if (listDataArray.length() == 0) {
            DialogHelper.showWarningDialog(CompactingRollWiseActivity.this, "Message", "Please scan bundle before data saving..");
        } else {
            progressBarState();
            finishProductionViewModel.postFinishProductionCompactingResponse(body).observe(this, apiResponse -> {
                if(apiResponse != null){
                    if(apiResponse.getStatus()){
                        DialogHelper.showSuccessDialog(CompactingRollWiseActivity.this, "Message", apiResponse.getResultset().getSaveMsg());
                        refreshDataFromUI();
                    }else{
                        DialogHelper.showWarningDialog(CompactingRollWiseActivity.this, "Message", apiResponse.getResultset().getSaveMsg());
                    }
                }else{
                    DialogHelper.showErrorDialog(CompactingRollWiseActivity.this, "Message", "Something wrong happen!");
                }
            });
        }
    }
    ...
    private void refreshDataFromUI() {
        ...
        finishProductionCompactingRecyclerAdapter.notifyDataSetChanged();
        ...
        _lengthShrinkageET.setText("");
        _widthShrinkageET.setText("");
        _spiralityET.setText("");
        _temperatureET.setText("");
        _speedET.setText("");
        _steamET.setText("");
        _overFeedET.setText("");
    }
...
//                intent1.putExtra("qc", "compacting_roll_wise");
...
//                    } else if (compactBatchScanResponse == null || compactBatchScanResponse.getResultset() == null || compactBatchScanResponse.getResultset().getDtlsIndex() == null || compactBatchScanResponse.getResultset().getDtlsIndex().isEmpty()) {
...
//                        intent.putExtra("qc", "compacting_roll_wise");
//                        intent.putExtra("scan_op", 2);
//                        intent.putExtra("batch_scan", text);
...
//                }
    ...
    public void setUp() {
        ...
        mToolbar.setTitle(R.string.compacting_roll);
...
//        _processEndTime.setOnClickListener(this);
        ...
        _temperatureET = findViewById(R.id.temparatureET);
        ...
        _reCompactingET = findViewById(R.id.reCompactingET);
...
//        _processStartTime.setText(currentTime);
    ...
    private void setScenedInformation() {
        if (compactBatchScanResponse != null
                && compactBatchScanResponse.getResultset() != null
                && compactBatchScanResponse.getResultset().getDtlsIndex() != null
                && compactBatchScanResponse.getResultset().getDtlsIndex().size() > 0) {

            _noRollTV.setText(String.valueOf(compactBatchScanResponse.getResultset().getDtlsIndex().size()));

            int checked = 0;
            for (int i = 0; i < compactBatchScanResponse.getResultset().getDtlsIndex().size(); i++) {
                if (compactBatchScanResponse.getResultset().getDtlsIndex().get(i).getBarcode_status() || compactBatchScanResponse.getResultset().getDtlsIndex().get(i).getChecked().equals("1")) {
                    checked++;
                }
            }

            _scannedNoRollTV.setText(String.valueOf(checked));
            _blRollTV.setText(String.valueOf(compactBatchScanResponse.getResultset().getDtlsIndex().size() - checked));
        } else {
    ...
    private void saveLocalDataData() {
        SharedPreferences sharedPreferences = getSharedPreferences("CompactingPref", MODE_PRIVATE);
        ...
        editor.putString("length_shri", _lengthShrinkageET.getText().toString());
        editor.putString("overfeed", _overFeedET.getText().toString());
        editor.putString("width_shri", _widthShrinkageET.getText().toString());
        editor.putString("spirality", _spiralityET.getText().toString());
        editor.putString("temp", _temperatureET.getText().toString());
    ...
    private void loadData() {
        ...
        _lengthShrinkageET.setText(sharedPreferences.getString("length_shri", ""));
        _overFeedET.setText(sharedPreferences.getString("overfeed", ""));
        _widthShrinkageET.setText(sharedPreferences.getString("width_shri", ""));
        _spiralityET.setText(sharedPreferences.getString("spirality", ""));
        _temperatureET.setText(sharedPreferences.getString("temp", ""));
