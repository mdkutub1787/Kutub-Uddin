...
public class V1_GreyRollReceiveActivity extends AppCompatActivity implements View.OnClickListener, V1_GreyRollReceiveRecyclerViewAdapter.OnMoreHeadListener, V1_GreyRollReceiveRecyclerViewAdapter.OnRemoveHeadListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_grey_roll_receive);
    ...
    private void getDefaultData() {
        ...
        if(grey_room_rack_scan != null || grey_scan_op == 1){
            assert grey_room_rack_scan != null;
            String[] roomRackArray = grey_room_rack_scan.split(Pattern.quote("***"));
//            String[] roomRackArray = grey_room_rack_scan.split(Pattern.quote("-"));  //palmal
//            String[] roomRackArray = grey_room_rack_scan.split("-", 2); // Limit to 2 parts
            ...
            if(apiResponse != null){
                try {
                    ...
                    for (int i = 0; i < apiResponse.getResultset().getDtlsPart().size(); i++) {
                        ...
                        greyRollReceiveItemModel.setCompanyId(apiResponse.getResultset().getMasterPart().getCompanyId());
                        greyRollReceiveItemModel.setDelivery_number(apiResponse.getResultset().getMasterPart().getSysNumber());
                        ...
                        greyRollReceiveItemModel.setInternalRef(apiResponse.getResultset().getDtlsPart().get(i).getInternalRef());
            ...
            if(apiResponse != null){
                if(!apiResponse.getResultset().getMasterPart().getStatus().equals("Failed")){
                    if (apiResponse.getResultset().getDtlsPart().size() > 0) {
                        ...
                        greyRollReceiveRecyclerViewAdapter.notifyDataSetChanged();
                        calculateTotalRollWeight();
                        handler.postDelayed(() -> startScanning(2, 2), SPLASH_DISPLAY_LENGTH);
                    }
                }else{
    ...
    private void calculateTotalRollWeight() {
        ...
        if(greyRollReceiveItemModels != null && greyRollReceiveItemModels.size() > 0) {
            _totalRollTV.setText("Total No of Roll : " + greyRollReceiveItemModels.size());
        }
    }
    ...
    private void postDataToServer() {
        ...
        try {
            ...
            master_obj.put("COMPANY_ID", greyRollReceiveItemModels.get(0).getCompanyId());
            master_obj.put("USER_ID", user_id);
            master_obj.put("RECEIVE_DATE", _receiveDateTV.getText().toString());
            master_obj.put("DELIVERY_NUMBER", greyRollReceiveItemModels.get(0).getDelivery_number());
        ...
        grayProductionViewModel.postGrayRollReceiveResponse(body).observe(this, apiResponse -> {
            if(apiResponse != null){
                if(apiResponse.getResultset().getMasterPart().getStatus().equals("Failed")){
                    DialogHelper.showWarningDialog(this, "Warning", apiResponse.getResultset().getMasterPart().getMsg());
                }else{
    ...
    private void refreshData(int status) {
        ...
        _totalWeightTV.setText("Total Weight: ");
        if(status == 3){
    ...
    private void showAlertMessage(String msg, int i, int position) {
        AlertDialog.Builder builder = new AlertDialog.Builder(V1_GreyRollReceiveActivity.this);
