...
import com.logicsoftbd.lsl.data.network.v1_model.V1_DefectListOFRectifiedModel_V4;
...
public class V1_StyleWiseSewingActivity_v4 extends AppCompatActivity implements View.OnClickListener,
    ...
    public static ArrayList<V1_DefectedRectifiedModel_V4> defectedRectifiedModels = new ArrayList<>();
    ...
    private V1_DefectedRectifiedRecyclerViewAdapter_v4 defectedRectifiedRecyclerViewAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v1_style_wise_sewing_v4);
...
//            poColorWiseSizeArrayList.get(savedSizeSelection).setBadgeQuantity(String.valueOf(Integer.parseInt(poColorWiseSizeArrayList.get(savedSizeSelection).getBadgeQuantity())-1));
        ...
        if(rectified_number == 1){
            ...
            for(int i = 0; i < defectedRectifiedModels.size(); i++)
            {
                if(defectedRectifiedModels.get(i).getSelect()){
                    ...
                    dtls_obj.put("color_size_id", defectedRectifiedModels.get(i).getColSizeId());
                    dtls_obj.put("qnty", 1);
    ...
    private JSONObject buildJsonObject_rectified() throws JSONException{
        ...
        for(int i = 0; i < defectedRectifiedModels.size(); i++)
        {
            if(defectedRectifiedModels.get(i).getSelect()){
                ...
                dtls_obj.put("color_size_id", defectedRectifiedModels.get(i).getColSizeId());
                dtls_obj.put("dtls_id", defectedRectifiedModels.get(i).getDtlsId());
                dtls_obj.put("mst_id", defectedRectifiedModels.get(i).getMstId());
                dtls_obj.put("rectified_qnty", defectedRectifiedModels.get(i).getDefectCount());
                dtls_arr.put(dtls_obj);
            }
        }
    ...
    private void showAlertMessage(String msg, Integer i) {
        AlertDialog.Builder builder = new AlertDialog.Builder(V1_StyleWiseSewingActivity_v4.this);
...
//        }
        ...
        apiInterface.getColorBreakDownClassCall(coloSizeBreakDownId, String.valueOf(lineId), po_breakDownId, item_numberId, countryId , userId).enqueue(new Callback<V1_ColorBreakDownResponse>() {
            ...
            public void onFailure(Call<V1_ColorBreakDownResponse> call, Throwable t) {
                hideDialog();
                Log.d(TAG, "onFailure: "+t.getMessage());
                Toast.makeText(V1_StyleWiseSewingActivity_v4.this, "Can't get color size data.", Toast.LENGTH_SHORT).show();
            }
        });
...
//                            }
    ...
    private void requestForRectifiedData() {
        ...
        apiInterface.getDefectedListForRectifiedModelCall_V4(selectedColorID, String.valueOf(lineId), userId, po_breakDownId, item_numberId, jobId).enqueue(new Callback<V1_DefectListOFRectifiedModel_V4>() {
            @RequiresApi(api = Build.VERSION_CODES.N)
            @Override
            public void onResponse(Call<V1_DefectListOFRectifiedModel_V4> call, Response<V1_DefectListOFRectifiedModel_V4> response) {
                Log.d(TAG, "onResponse: "+response.toString());
                if(response.isSuccessful()){
                    defectedRectifiedModels.clear();
            ...
            @Override
            public void onFailure(Call<V1_DefectListOFRectifiedModel_V4> call, Throwable t) {
                Log.d(TAG, "onFailure: "+t.getMessage());
                Toast.makeText(V1_StyleWiseSewingActivity_v4.this, "Failed", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void initRectifiedDefectRecyclerView() {
        ...
        Collections.sort(defectedRectifiedModels, V1_DefectedRectifiedModel_V4.nameComparator);
        defectedRectifiedRecyclerViewAdapter = new V1_DefectedRectifiedRecyclerViewAdapter_v4( this, defectedRectifiedModels,  this);
