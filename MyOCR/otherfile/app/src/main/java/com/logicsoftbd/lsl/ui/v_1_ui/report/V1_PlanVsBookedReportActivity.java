...
import com.logicsoftbd.lsl.data.network.v1_model.V1_PlanVsBookedModel;
...
public class V1_PlanVsBookedReportActivity extends AppCompatActivity implements View.OnClickListener{
    private static final String TAG = "PlanVsBookedReportActiv";
    ...
    public String[] planMinuteArray, bookedMinuteArray;
    public static ArrayList<V1_PlanVsBookedModelClass> modelArrayList;
    private V1_PlanVsBookedAdapter planVsBookedAdapter;
    private GridView planBookedGridView;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_v1_plan_vs_booked_report);
        ...
        alertDialogBuilder = new android.app.AlertDialog.Builder(V1_PlanVsBookedReportActivity.this);
        ...
        apiInterface.getBundkeWiseSewingInputClassCall().enqueue(new Callback<V1_BundleWiseSewingInputClass>() {
            ...
            public void onFailure(Call<V1_BundleWiseSewingInputClass> call, Throwable t) {
                if (t instanceof IOException) {
                    Toast.makeText(V1_PlanVsBookedReportActivity.this, "No network available, please check your WiFi or Data connection", Toast.LENGTH_SHORT).show();
                    hideDialog();
                }
                else {
    ...
    private void setAdapterData_Location() {
        ArrayAdapter<String> adapterLocation = new ArrayAdapter<String>(V1_PlanVsBookedReportActivity.this, R.layout.sewing_spinner_layout, locationNameArray);
    ...
    private void showReportFromServer() {
        showDialog();
        apiInterface.getPlanVsBookedModelCall(companyId, locationId, startButton.getText().toString(), endButton.getText().toString()).enqueue(new Callback<V1_PlanVsBookedModel>() {
            @Override
            public void onResponse(Call<V1_PlanVsBookedModel> call, Response<V1_PlanVsBookedModel> response) {
                if(response.isSuccessful()){
                    hideDialog();
                    plan_vs_booked.setVisibility(View.GONE);
                    monthNameTV.setText(String.valueOf(response.body().getData().get(0).getMonth()));
                    V1_PlanVsBookedModel.Datum rCompanyName;
                    final List<V1_PlanVsBookedModel.Datum> CompanyName = response.body().getData();
                    for(V1_PlanVsBookedModel.Datum d : CompanyName)
                    {
                        rCompanyName = d;
                        final V1_PlanVsBookedModel.Datum finalName = rCompanyName;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedModel.Datum rLocationName;
                    final List<V1_PlanVsBookedModel.Datum> LocationName = response.body().getData();
                    for(V1_PlanVsBookedModel.Datum d : LocationName)
                    {
                        rLocationName = d;
                        final V1_PlanVsBookedModel.Datum finalName = rLocationName;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedModel.Datum rMonth;
                    final List<V1_PlanVsBookedModel.Datum> MonthName = response.body().getData();
                    for(V1_PlanVsBookedModel.Datum d : MonthName)
                    {
                        rMonth = d;
                        final V1_PlanVsBookedModel.Datum finalName = rMonth;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedModel.Datum rPlanMinute;
                    final List<V1_PlanVsBookedModel.Datum> PlanMinute = response.body().getData();
                    for(V1_PlanVsBookedModel.Datum d : PlanMinute)
                    {
                        rPlanMinute = d;
                        final V1_PlanVsBookedModel.Datum finalName = rPlanMinute;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedModel.Datum rBookedMinute;
                    final List<V1_PlanVsBookedModel.Datum> BookedMinute = response.body().getData();
                    for(V1_PlanVsBookedModel.Datum d : BookedMinute)
                    {
                        rBookedMinute = d;
                        final V1_PlanVsBookedModel.Datum finalName = rBookedMinute;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    planVsBookedAdapter = new V1_PlanVsBookedAdapter(V1_PlanVsBookedReportActivity.this, R.layout.plan_vs_booked_report_layout, modelArrayList);
                    planBookedGridView.setAdapter(planVsBookedAdapter);
            ...
            @Override
            public void onFailure(Call<V1_PlanVsBookedModel> call, Throwable t) {
                hideDialog();
            }
        });
    }

    private ArrayList<V1_PlanVsBookedModelClass> getModel() {
        ArrayList<V1_PlanVsBookedModelClass> list = new ArrayList<>();
        ...
        for(int i = 0; i < rCompanyNameArray.length; i++){
            V1_PlanVsBookedModelClass planVsBookedModelClass = new V1_PlanVsBookedModelClass();
            planVsBookedModelClass.setCompanyName(rCompanyNameArray[i]);
            planVsBookedModelClass.setLocationName(rLocationNameArray[i]);
            planVsBookedModelClass.setMonth(monthNameArray[i]);
            planVsBookedModelClass.setPlan(String.valueOf(planMinuteArray[i]));
            planVsBookedModelClass.setBooked(String.valueOf(bookedMinuteArray[i]));
            list.add(planVsBookedModelClass);
        }
        return list;
    }
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent intent = new Intent(V1_PlanVsBookedReportActivity.this, V1_MenuActivity.class);
    ...
    private void datepicker(final Button setdate) {
        ...
        // date picker dialog
        datePickerDialog = new DatePickerDialog(V1_PlanVsBookedReportActivity.this,
