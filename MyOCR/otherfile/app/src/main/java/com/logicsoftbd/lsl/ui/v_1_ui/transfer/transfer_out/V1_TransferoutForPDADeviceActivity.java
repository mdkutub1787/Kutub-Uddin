...
import com.logicsoftbd.lsl.databinding.ActivityV1TransferoutPdaDeviceBinding;
...
public class V1_TransferoutForPDADeviceActivity extends AppCompatActivity implements DeleteItemFromTransferOutInterface, AdapterView.OnItemSelectedListener {
    private static final String TAG = "V1_TransferoutForPDADev";
    public ActivityV1TransferoutPdaDeviceBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityV1TransferoutPdaDeviceBinding.inflate(getLayoutInflater());
        ...
        binding.locationScanBT.setOnClickListener(view -> {
            currentIdentifier = "location";
            binding.locationScanBT.setBackgroundColor(ContextCompat.getColor(this, R.color.red));
            binding.barcodeScanBT.setBackgroundColor(Color.parseColor("#324F5C"));
//                    startScanning(1));
        });
...
//                startScanning(2);
                ...
                binding.barcodeScanBT.setBackgroundColor(ContextCompat.getColor(this, R.color.red));
                binding.locationScanBT.setBackgroundColor(Color.parseColor("#324F5C"));
            }
        });
    ...
    private void callAdapterForSpinner(ArrayList<String> storeNameList) {
        spinnerAdapter = new ArrayAdapter<>(V1_TransferoutForPDADeviceActivity.this, android.R.layout.simple_spinner_item, storeNameList);
...
//                requestForBarcode(barcode_scan_result, separated_location_id, listOfAllBarcode);
                requestForBarcode(barcode_scan_result, separated_location_id);
            }
        } else {
            // no implementation
        }
        ...
        apiInterface.store_name_list_for_grey_fabric_transfer_out(user_id, category_item_id, company_id).enqueue(new Callback<V1GreyFabricTransferOutStoreListResponse>() {
            ...
            public void onResponse(Call<V1GreyFabricTransferOutStoreListResponse> call, Response<V1GreyFabricTransferOutStoreListResponse> response) {
                ...
                if(response.isSuccessful() && response.body().getStores() != null && response.body().getStores().size() > 0) {
                    ...
                    DialogHelper.showWarningDialog(V1_TransferoutForPDADeviceActivity.this, "Message", "Store not found.");
                }
            }
            ...
            public void onFailure(Call<V1GreyFabricTransferOutStoreListResponse> call, Throwable t) {
                ...
                DialogHelper.showErrorDialog(V1_TransferoutForPDADeviceActivity.this, "Message", "Something went wrong.");
            }
        });
    }
...
//    }
    ...
    private void requestForBarcode(String barcodeScan, String separated_location_id) {
        setVisibilityForMainProgressBar(View.VISIBLE);
        setVisibilityForFullLayout(View.GONE);
                    ...
                    if (response.isSuccessful()) {
                        ...
                        if ((response.body().getStatus()).equals("200")) {
                            if (po_breakdown_id.equals("") && floor_id.equals("") && room_id.equals("") && rack_id.equals("") && shelf_id.equals("") && bin_id.equals("")) {
                                ...
                                bin_id = response.body().getResultset().getDtlsPart().getbIN_ID();
                                item_category_id = response.body().getResultset().getDtlsPart().getItemCategoryId();
                                addNewBarcodeInListAfterCheckAllId(response);
                                requestForStoreList(user_id, item_category_id, company_id);
                            } else if (po_breakdown_id.equals(response.body().getResultset().getDtlsPart().getPOBREAKDOWNID()) &&
                                    floor_id.equals(response.body().getResultset().getDtlsPart().getfLOOR_ID()) &&
                                    room_id.equals(response.body().getResultset().getDtlsPart().getrOOM_ID()) &&
...
//        listOfBarcodeOnlySingleUI.clear();
    ...
    private void showMessageWithSweetAlert(String msg) {
        new SweetAlertDialog(V1_TransferoutForPDADeviceActivity.this, SweetAlertDialog.WARNING_TYPE)
...
//    }
    ...
    @SuppressLint("NotifyDataSetChanged")
    private void addNewBarcodeInListAfterCheckAllId(Response<TransferOutResponse> response) {
//        listOfBarcodeOnlySingleUI.clear();
//        listOfBarcodeOnlySingleUI = listOfAllBarcode;
        ...
        if (alreadyTaken) {
            ...
            company_id = response.body().getResultset().getMasterPart().getCOMPANYID() + "";
            listOfBarcodeOnlySingleUI.add(single_dtlsPart);
        }
        callAdapter((ArrayList<DtlsPart>) listOfBarcodeOnlySingleUI);
    }
    ...
    private void callAdapter(ArrayList<DtlsPart> listOfAllBarcode) {
        adapter = new V1_transfer_out_recycler_adapter(V1_TransferoutForPDADeviceActivity.this, listOfAllBarcode);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_save) {
            showAlertMessageForGettingPermission("Do you want to save?");
            return true;
        }
                ...
                if (currentIdentifier != null) {
                    switch (currentIdentifier) {
                        case "location":{
                            if(barcodeData != null){
                                try {
                                    String[] location_split_array = barcodeData.split(Pattern.quote("***"));
                                    separated_location_id = location_split_array[0];
                                    separated_location_name = location_split_array[1];
                                    binding.locationScanTV.setText(separated_location_name);
                                } catch (Exception e) {
                        ...
                        case "bundle":
                            if(barcodeData != null){
                                ...
                                binding.barcodeScanTV.setText(barcodeData);
                                binding.locationScanTV.setText(separated_location_name);

                                binding.locationScanBT.setClickable(false);
                                //request for barcode and set list data
//                                requestForBarcode(barcodeData, separated_location_id, listOfAllBarcode);
                                requestForBarcode(barcodeData, separated_location_id);
                            }
                            break;
                        default:
