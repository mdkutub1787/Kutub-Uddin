...
public class StenteringRollWiseActivity extends AppCompatActivity implements View.OnClickListener, FinishProductionStentaringRecyclerAdapter.OnSelectSelectListener {
    private static final String TAG = "StenteringRollWiseActiv";
    ...
    private EditText _batchNoScanET, _barcodeScanET, _overFeedET, _stretchET, _feedInET, _pinningET, _speedET, _temparatureET, _productionTimeHour, _productionTimeMin, _processStartTimeHour, _processStartTimeMin, _processEndTimeHour, _processEndTimeMin, _reStenteringET;
    ...
    private FinishProductionStentaringRecyclerAdapter finishProductionRecyclerAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_stentering_roll_wise);
    ...
    private void fetchCompactingData() {
        progressBarState();
        finishProductionViewModel.getFinishProductionDefaultResponse("48").observe(this, apiResponse -> {
            if(apiResponse != null){
                ...
                DialogHelper.showErrorDialog(StenteringRollWiseActivity.this, "Message", "Something went wrong!");
            }
        });
    }
    ...
    private void fetchCompactingBatchScan(String scannedBatch) {
        progressBarState();
        finishProductionViewModel.getFinishProductionStenteringBatchScanResponse(scannedBatch, "48").observe(this, apiResponse -> {
            if(apiResponse!= null){
                if(apiResponse.getResultset() != null) {
                    ...
                    stenteringBatchScanResponse = apiResponse;
    ...
    private void setDataToUI(StenteringBatchScanResponse response) {
        ...
        if(response.getResultset().getInputAreaIndex().getReSlittingNo().equals("false")){
            _reStenteringET.setText("0");
        }else{
            _reStenteringET.setText(response.getResultset().getInputAreaIndex().getReSlittingNo());
        }
    ...
    private void setupCompactingProdScanRecyclerView() {
        ...
        finishProductionRecyclerAdapter = new FinishProductionStentaringRecyclerAdapter( dtlsIndexArrayList, this, this);
    ...
    private void saveRequestCompactingObject() throws JSONException {
        ...
        index_obj.put("RE_SLITING_NO", stenteringBatchScanResponse.getResultset().getInputAreaIndex().getReSlittingNo());
...
//        index_obj.put("END_MINUTES", _processEndTime.getText().toString().split(":")[1]);
        ...
        if (listDataArray.length() == 0) {
            DialogHelper.showWarningDialog(StenteringRollWiseActivity.this, "Message", "Please scan bundle before data saving..");
        } else {
            progressBarState();
            finishProductionViewModel.postFinishProductionCompactingResponse(body).observe(this, apiResponse -> {
                if(apiResponse != null){
                    if(apiResponse.getStatus()){
                        DialogHelper.showSuccessDialog(StenteringRollWiseActivity.this, "Message", apiResponse.getResultset().getSaveMsg());
                        refreshDataFromUI();
                    }else{
                        DialogHelper.showWarningDialog(StenteringRollWiseActivity.this, "Message", apiResponse.getResultset().getSaveMsg());
                    }
                }else{
                    DialogHelper.showErrorDialog(StenteringRollWiseActivity.this, "Message", "Something wrong happen!");
                }
            });
        }
    }
...
//                intent1.putExtra("qc", "stentering_roll_wise");
...
//                    } else if (stenteringBatchScanResponse == null || stenteringBatchScanResponse.getResultset() == null || stenteringBatchScanResponse.getResultset().getDtlsIndex() == null || stenteringBatchScanResponse.getResultset().getDtlsIndex().isEmpty()) {
...
//                        intent.putExtra("qc", "stentering_roll_wise");
...
//                }
    ...
    public void setUp() {
        ...
        mToolbar.setTitle(R.string.stentering_roll);
...
//        _processEndTime.setOnClickListener(this);
        ...
        _reStenteringET = findViewById(R.id.reStenteringET);
...
//        _processStartTime.setText(currentTime);
        ...
        if(barcodeScan != null && op == 2){
            ...
            try {
                ...
                Log.d(TAG, "setDataUI: "+e.getMessage());
                DialogHelper.showWarningDialog(this, "Message", "Please try again.");
            }
        }
    ...
    private void setScenedInformation() {
        if (stenteringBatchScanResponse != null
                && stenteringBatchScanResponse.getResultset() != null
                && stenteringBatchScanResponse.getResultset().getDtlsIndex() != null
                && stenteringBatchScanResponse.getResultset().getDtlsIndex().size() > 0) {

            _noRollTV.setText(String.valueOf(stenteringBatchScanResponse.getResultset().getDtlsIndex().size()));

            int checked = 0;
            for (int i = 0; i < stenteringBatchScanResponse.getResultset().getDtlsIndex().size(); i++) {
                if (stenteringBatchScanResponse.getResultset().getDtlsIndex().get(i).getBarcode_status() || stenteringBatchScanResponse.getResultset().getDtlsIndex().get(i).getChecked().equals("1")) {
                    checked++;
                }
            }

            _scannedNoRollTV.setText(String.valueOf(checked));
            _blRollTV.setText(String.valueOf(stenteringBatchScanResponse.getResultset().getDtlsIndex().size() - checked));
        } else {
    ...
    private void saveLocalDataData() {
        SharedPreferences sharedPreferences = getSharedPreferences("StenteringPref", MODE_PRIVATE);
...
//        editor.putInt("shift_position", Integer.parseInt(shiftId));
        ...
        editor.putString("stretch", _stretchET.getText().toString());
        editor.putString("overfeed", _overFeedET.getText().toString());
        editor.putString("feedin", _feedInET.getText().toString());
        editor.putString("pinning", _pinningET.getText().toString());
    ...
    private void loadData() {
        ...
        _stretchET.setText(sharedPreferences.getString("stretch", ""));
        _overFeedET.setText(sharedPreferences.getString("overfeed", ""));
        _feedInET.setText(sharedPreferences.getString("feedin", ""));
        _pinningET.setText(sharedPreferences.getString("pinning", ""));
        _temparatureET.setText(sharedPreferences.getString("temp", ""));
        _speedET.setText(sharedPreferences.getString("speed", ""));
        _productionTimeHour.setText(sharedPreferences.getString("production_hour", ""));
        _productionTimeMin.setText(sharedPreferences.getString("production_min", ""));
