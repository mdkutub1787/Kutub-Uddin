...
public class FinishFabricResultSetActivity extends BaseActivity implements ReceiveMvpView, DefectListAdapter.Callback, ButtonListAdapter.Callback, DatePickerDialog.OnDateSetListener {
    ...
    Spinner spinner_shade;
    private Date mSelectedDate;
    @Inject
    ReceiveMvpPresenter<ReceiveMvpView, ReceiveMvpInteractor> mPresenter;
    private FinishFabricResultSet mBarcodeResponse;
    ArrayList<SpinnerModel> mArrayList = new ArrayList<>();
    ArrayAdapter<SpinnerModel> mArrayAdapter;
    ArrayAdapter<FabricShade.Result> mArrayShadeAdapter;
    String status = "";
    String shade = "";
    ArrayList<FabricShade.Result> spinnerFabricArrayList = new ArrayList<>();
    public static Intent getStartIntent(Context context, FinishFabricResultSet process) {
        Intent intent = new Intent(context, FinishFabricResultSetActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_finish_fabric_result_set);
        ...
        mPresenter.getDefectInch();
        mPresenter.getDefectList();
        ...
        if (linear.getChildCount()> 0)
        {
            for (int j = 0; j < linear.getChildCount(); j++) {
                ...
                if (edit_count.getText().toString().equals("")) {
                    return false;
                } else if (edit_defect_found_inch.getText().toString().equals("")) {
                    ...
                    detailsPart.setDEFECT_COUNT(edit_count.getText().toString());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_save) {
            if(status.equals("0")){
            ...
            else if(shade.equals("-1")){
                Toast.makeText(this, "Please Select Fabric Shade", Toast.LENGTH_SHORT).show();
            }
            else{
              if (edit_qc_name.getText().toString().equals("")) {
                ...
                if (getData()) {
                    ...
                    masterPart.setAC_GSM(edit_gsm.getText().toString());
                    masterPart.setROLL_LENGTH(edit_roll_length.getText().toString());

                    if(edit_comments.getText().toString().equals("")){
                        masterPart.setCOMMENTS("");
                    }
                    else{
                        masterPart.setCOMMENTS(edit_comments.getText().toString());
                    }
                    masterPart.setFABRIC_SHADE(shade);
                    int l = 0;
                    ...
                    mPresenter.onResultSetSave(resultEntryRequest);
            ...
            startActivity(new Intent(FinishFabricResultSetActivity.this, V1_MenuActivity.class));
            finish();
    ...
    protected void setUp() {
        ...
        mArrayShadeAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, spinnerFabricArrayList);
        mArrayShadeAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinner.setAdapter(mArrayAdapter);
        spinner_shade.setAdapter(mArrayShadeAdapter);
        mBarcodeResponse = (FinishFabricResultSet) getIntent().getSerializableExtra(EXTRA_RECEIVE_ID);
        try {
            edit_barcode.setText(mBarcodeResponse.getData().getBARCODE_NO());
            edit_roll_weight.setText(mBarcodeResponse.getData().getQNTY());
            edit_gsm.setText(mBarcodeResponse.getData().getGSM());
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        mToolbar.setTitle(R.string.finish_fabric_entry);
        ...
        spinner_shade.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                Log.e("sp_water", "" + spinnerFabricArrayList.get(position).getID());
                shade = String.valueOf(spinnerFabricArrayList.get(position).getID());
            }
...
//                            double leg=(2*1000)/((2*5*2)*1);
    ...
    public void onSuccess(String msg) {
        showAlertDialog(new DialogButtonClickListener() {
            ...
            public void onButtonClick() {
                //  finish();
                startActivity( ScannerActivity.getStartIntent(FinishFabricResultSetActivity.this, new Process(R.drawable.process, "Finish Fabric Production Result Entry", "result",
    ...
    public void onFabricShade(FabricShade fabricGrade) {
        Log.e("FabricShade", "FabricShade" + new Gson().toJson (fabricGrade));
        FabricShade.Result fabricGrades = new FabricShade.Result();
        fabricGrades.setID(-1);
        fabricGrades.setSTORE_NAME("Select");
        spinnerFabricArrayList.add(fabricGrades);
        spinnerFabricArrayList.addAll(fabricGrade.getData());
        mArrayShadeAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, spinnerFabricArrayList);
        mArrayShadeAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinner_shade.setAdapter(mArrayShadeAdapter);
    }
        ...
        if (linear.getChildCount() > 0) {
            ...
            if (editDefectFoundInch.getText().toString().length() > 0) {
                if (!editDefectFoundInch.getText().toString().equals("")) {
                    ...
                    int total = 0;
                    if (!edit_count.getText().toString().equals("")) {
                        i = Integer.parseInt(edit_count.getText().toString());
                    } else {
                        i = 0;
                    }
