...
import com.logicsoftbd.lsl.data.network.v1_model.V1_PlanVsBookedVsCapacityModel;
import com.logicsoftbd.lsl.data.network.v1_model.V1_PlanVsBookedVsCapacityModelClass;
...
public class V1_PlanVsBookedVsCapacityActivity extends AppCompatActivity implements View.OnClickListener {
    private static final String TAG = "PlanVsBookedVsCapacityA";
    ...
    private TextView monthNameTV, planFirstTV, planTotalTV, bookedFirstTV, bookedTotalTV, capacityFirstTV, capacityTotalTV, balanceFirstTV, balanceTotalTV;
    ...
    public final ArrayList<String> bookedMinuteList = new ArrayList<>();
    public final ArrayList<String> capacityMinuteList = new ArrayList<>();

    public String[] rCompanyNameArray, rLocationNameArray, monthNameArray, planMinuteArray, bookedMinuteArray, capacityMinuteArray;
    public static ArrayList<V1_PlanVsBookedVsCapacityModelClass> modelArrayList;
    private V1_PlanVsBookedVsCapacityAdapter planVsBookedVsCapacityAdapter;
    private GridView planBookedGridView;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_v1_plan_vs_booked_vs_capacity);
        ...
        alertDialogBuilder = new android.app.AlertDialog.Builder(V1_PlanVsBookedVsCapacityActivity.this);
    ...
    private void initialization() {
        ...
        capacityFirstTV = findViewById(R.id.capacityfirstText);
        capacityTotalTV = findViewById(R.id.capacityTotalText);
        ...
        apiInterface.getBundkeWiseSewingInputClassCall().enqueue(new Callback<V1_BundleWiseSewingInputClass>() {
            ...
            public void onFailure(Call<V1_BundleWiseSewingInputClass> call, Throwable t) {
                if (t instanceof IOException) {
                    Toast.makeText(V1_PlanVsBookedVsCapacityActivity.this, "No network available, please check your WiFi or Data connection", Toast.LENGTH_SHORT).show();
                    hideDialog();
                }
                else {
    ...
    private void setAdapterData_Location() {
        ArrayAdapter<String> adapterLocation = new ArrayAdapter<String>(V1_PlanVsBookedVsCapacityActivity.this, R.layout.sewing_spinner_layout, locationNameArray);
    ...
    private void showReportFromServer() {
        showDialog();
        apiInterface.getPlanVsBookedVsCapacityModelCall(companyId, locationId, startButton.getText().toString(), endButton.getText().toString()).enqueue(new Callback<V1_PlanVsBookedVsCapacityModel>() {
            @Override
            public void onResponse(Call<V1_PlanVsBookedVsCapacityModel> call, Response<V1_PlanVsBookedVsCapacityModel> response) {
                if(response.isSuccessful()){
                    hideDialog();
                    plan_vs_booked.setVisibility(View.GONE);
                    monthNameTV.setText(String.valueOf(response.body().getData().get(0).getMonth()));
                    V1_PlanVsBookedVsCapacityModel.Datum rCompanyName;
                    final List<V1_PlanVsBookedVsCapacityModel.Datum> CompanyName = response.body().getData();
                    for(V1_PlanVsBookedVsCapacityModel.Datum d : CompanyName)
                    {
                        rCompanyName = d;
                        final V1_PlanVsBookedVsCapacityModel.Datum finalName = rCompanyName;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedVsCapacityModel.Datum rLocationName;
                    final List<V1_PlanVsBookedVsCapacityModel.Datum> LocationName = response.body().getData();
                    for(V1_PlanVsBookedVsCapacityModel.Datum d : LocationName)
                    {
                        rLocationName = d;
                        final V1_PlanVsBookedVsCapacityModel.Datum finalName = rLocationName;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedVsCapacityModel.Datum rMonth;
                    final List<V1_PlanVsBookedVsCapacityModel.Datum> MonthName = response.body().getData();
                    for(V1_PlanVsBookedVsCapacityModel.Datum d : MonthName)
                    {
                        rMonth = d;
                        final V1_PlanVsBookedVsCapacityModel.Datum finalName = rMonth;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedVsCapacityModel.Datum rPlanMinute;
                    final List<V1_PlanVsBookedVsCapacityModel.Datum> PlanMinute = response.body().getData();
                    for(V1_PlanVsBookedVsCapacityModel.Datum d : PlanMinute)
                    {
                        rPlanMinute = d;
                        final V1_PlanVsBookedVsCapacityModel.Datum finalName = rPlanMinute;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    V1_PlanVsBookedVsCapacityModel.Datum rBookedMinute;
                    final List<V1_PlanVsBookedVsCapacityModel.Datum> BookedMinute = response.body().getData();
                    for(V1_PlanVsBookedVsCapacityModel.Datum d : BookedMinute)
                    {
                        rBookedMinute = d;
                        final V1_PlanVsBookedVsCapacityModel.Datum finalName = rBookedMinute;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                bookedMinuteList.add(String.valueOf(finalName.getBookedMinute()));
                            }
                        });
                    }

                    V1_PlanVsBookedVsCapacityModel.Datum rCapacityMinute;
                    final List<V1_PlanVsBookedVsCapacityModel.Datum> CapacityMinute = response.body().getData();
                    for(V1_PlanVsBookedVsCapacityModel.Datum d : CapacityMinute)
                    {
                        rCapacityMinute = d;
                        final V1_PlanVsBookedVsCapacityModel.Datum finalName = rCapacityMinute;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    planVsBookedVsCapacityAdapter = new V1_PlanVsBookedVsCapacityAdapter(V1_PlanVsBookedVsCapacityActivity.this, R.layout.plan_vs_booked_report_layout, modelArrayList);
                    planBookedGridView.setAdapter(planVsBookedVsCapacityAdapter);
            ...
            @Override
            public void onFailure(Call<V1_PlanVsBookedVsCapacityModel> call, Throwable t) {
                hideDialog();
            }
        });
    }

    private ArrayList<V1_PlanVsBookedVsCapacityModelClass> getModel() {
        ArrayList<V1_PlanVsBookedVsCapacityModelClass> list = new ArrayList<>();
        ...
        bookedMinuteArray = new String[bookedMinuteList.size()];
        for(int i = 0; i < bookedMinuteList.size(); i++)
        {
            bookedMinuteArray[i] = bookedMinuteList.get(i);
        }
        ...
        double[] longBooked = new double[bookedMinuteArray.length];
        for (int i = 0; i < bookedMinuteArray.length; i++)
            longBooked[i] = Double.parseDouble(bookedMinuteArray[i]);
        ...
        double[] longCapacity = new double[capacityMinuteArray.length];
        for (int i = 0; i < capacityMinuteArray.length; i++)
            longCapacity[i] = Double.parseDouble(capacityMinuteArray[i]);

        Double capacitySum = Double.valueOf(0);
        for( Double num : longCapacity) {
            capacitySum = capacitySum +num;
        }
        ...
        capacityFirstTV.setText(String.valueOf(capacitySum));
        capacityTotalTV.setText(String.valueOf(capacitySum));

        balanceFirstTV.setText(String.valueOf(bookedsum-capacitySum-plansum));
        balanceTotalTV.setText(String.valueOf(bookedsum-capacitySum-plansum));

        for(int i = 0; i < rCompanyNameArray.length; i++){
            V1_PlanVsBookedVsCapacityModelClass planVsBookedVsCapacityModelClass = new V1_PlanVsBookedVsCapacityModelClass();
            planVsBookedVsCapacityModelClass.setCompanyName(rCompanyNameArray[i]);
            planVsBookedVsCapacityModelClass.setLocationName(rLocationNameArray[i]);
            planVsBookedVsCapacityModelClass.setMonth(monthNameArray[i]);
            planVsBookedVsCapacityModelClass.setPlan(planMinuteArray[i]);
            planVsBookedVsCapacityModelClass.setBooked(bookedMinuteArray[i]);
            planVsBookedVsCapacityModelClass.setCapacity(capacityMinuteArray[i]);
            list.add(planVsBookedVsCapacityModelClass);
        }
        return list;
    }
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent intent = new Intent(V1_PlanVsBookedVsCapacityActivity.this, V1_MenuActivity.class);
    ...
    private void datepicker(final Button setdate) {
        ...
        // date picker dialog
        datePickerDialog = new DatePickerDialog(V1_PlanVsBookedVsCapacityActivity.this,
