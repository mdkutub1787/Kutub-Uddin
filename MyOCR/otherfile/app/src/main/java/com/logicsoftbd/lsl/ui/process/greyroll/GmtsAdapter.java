...
public class GmtsAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    private List<EmbSpBarcodeResponse.BodyPart.DetailsPart> mProcessResponseList = new ArrayList<>();

    private boolean hasForward = true;

    public GmtsAdapter( List<EmbSpBarcodeResponse.BodyPart.DetailsPart> openSourceResponseList) {
        mProcessResponseList = openSourceResponseList;
    }

    public void hasForward(boolean val) {
        hasForward = val;
    }
    ...
    public void addItems(List<EmbSpBarcodeResponse.BodyPart.DetailsPart> repoList) {
        if(mProcessResponseList != null && mProcessResponseList.size() > 0)
    ...
    public class ViewHolder extends BaseViewHolder {
        ...
        public void onBind(final int position) {
            ...
            final EmbSpBarcodeResponse.BodyPart.DetailsPart repo = mProcessResponseList.get(position);
            ...
            if(repo.getQty() > 0) {
                textViewQty.setText(repo.getQty()+"");
            }
