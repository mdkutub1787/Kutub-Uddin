...
public class V1_StyleWiseSewingActivity_v3 extends AppCompatActivity implements View.OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v1_style_wise_sewing_v3);
...
//        inputModelClass.setOrder_id(Integer.parseInt(_preferences.getString("poBreakDownId", "")));
        ...
        if(!operationJson.equals("")){
            ...
            alertForApkUpdate("Device not configured, Please configure first.");
        }
    ...
    private void initColorSpinnerView() {
        ...
        colorSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                _goodTV.setText(uniqueColorList.get(position).getGood());
                _rejectTV.setText(uniqueColorList.get(position).getReject());
                _alterTV.setText(uniqueColorList.get(position).getAlter());
                _spotTV.setText(uniqueColorList.get(position).getSpot());
                _rectifiedTV.setText(uniqueColorList.get(position).getRectified());
...
//            }
                ...
                Log.d(TAG, "saveDataAction: "+alterDefectSelectStatus+" "+ spotDefectSelectStatus);
    ...
    private void postDataToServer_sewing_input() throws JSONException  {
        ...
        apiInterface.saveUpdateBundleSewingGrossOutputCall(body).enqueue(new Callback<V1_GrossSewingSaveResponse>() {
            ...
            public void onResponse(Call<V1_GrossSewingSaveResponse> call, Response<V1_GrossSewingSaveResponse> response) {
                hideDialog();
                Log.d(TAG, "onResponse: save #######"+response.toString());
                if(response.isSuccessful()){
...
//            poColorWiseSizeArrayList.get(savedSizeSelection).setBadgeQuantity(String.valueOf(Integer.parseInt(poColorWiseSizeArrayList.get(savedSizeSelection).getBadgeQuantity())-1));
    ...
    private void showAlertMessage(String msg, Integer i) {
        AlertDialog.Builder builder = new AlertDialog.Builder(V1_StyleWiseSewingActivity_v3.this);
...
//                            }
    ...
    private void requestForRectifiedData() {
        ...
        apiInterface.getDefectedListForRectifiedModelCall_V1(selectedColorID, String.valueOf(lineId), userId, po_breakDownId, item_numberId, jobId).enqueue(new Callback<V1_DefectListOFRectifiedModel>() {
                    ...
                    if(response.body().getData() != null){
                        for(int i=0; i<response.body().getData().size(); i++){
                            if(response.body().getData().get(0).getMstId().equals("0")){
                                ...
                                defectedRectifiedModel.setColorSizedId(response.body().getData().get(i).getCOL_SIZE_ID());
            ...
            public void onFailure(Call<V1_DefectListOFRectifiedModel> call, Throwable t) {
                Log.d(TAG, "onFailure: "+t.getMessage());
                Toast.makeText(V1_StyleWiseSewingActivity_v3.this, "Failed", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//        }
    ...
    private void alertForApkUpdate(String message){
        ...
        updateBtn.setOnClickListener(v -> {
            dialog.dismiss();
            startActivity(new Intent(this, V1_StyleWiseLineConfigActivity_v3.class));
            finish();
        });
