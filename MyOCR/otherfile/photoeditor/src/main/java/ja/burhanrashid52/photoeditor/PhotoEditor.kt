...
interface PhotoEditor {
    fun createImageRounded(boolean: Boolean, context: Context, width: Int, height: Int, colorCode: Int, name: String?): Bitmap? {
        val output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(output)
        val paintCicle = Paint()
        val paintText = Paint()
        val rect = Rect(0, 0, width, height)
        val rectF = RectF(rect)
        val density = context.resources.displayMetrics.density
        val roundPx = 100 * density
        paintCicle.color = colorCode
        paintCicle.isAntiAlias = true
        canvas.drawARGB(0, 0, 0, 0)

// Set Border For Circle
        paintCicle.style = Paint.Style.FILL
        paintCicle.strokeWidth = 2.0f
        ...
        if (name != null) {
            ...
            paintText.getTextBounds(name, 0, 0, rect)
//            val x: Float = cWidth / 2f + rect.width() / 2f + rect.left
//            val y: Float = cHeight / 2f + rect.height() / 2f + rect.bottom
            canvas.drawText(name, 16F, 22F, paintText)



//            canvas.drawText(name, 25F - 16F, 25F - 2F, paintText)
        }
        return output
    }
    /**
